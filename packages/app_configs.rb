package :app_configs do
  ...
  transfer 'resources/config', '~', :recursive => true do
  ...
  transfer 'resources/config/application.yml', '~/application.yml', :sudo => true, :render => true, :locals => { :rails_env => ENV["RAILS_ENV"] } do
    pre :install, 'sudo mkdir -p /data/CHANGEME/shared/config'
